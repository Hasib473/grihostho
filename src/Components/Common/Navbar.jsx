...
const Navbar = () => {
    ...
    const suggestions = useMemo(() => searchItems(searchTerm).slice(0, 6), [searchTerm]);
        ...
        <>
            <header className="w-full bg-white text-[#07162b]">
                <div className="mx-auto flex max-w-[1760px] flex-wrap items-center gap-x-3 gap-y-4 px-3 py-3 sm:gap-x-4 sm:px-5 sm:py-4 lg:flex-nowrap lg:justify-between lg:px-8 xl:px-12">
                    <Link to="/" className="order-1 flex shrink-0 items-center" aria-label="Grihostho home">
                        <img src={logo} alt="Grihostho" className="h-12 w-auto object-contain sm:h-14 lg:h-16" />
                    </Link>

                    <form onSubmit={handleSearch} className="relative order-3 flex h-11 w-full min-w-0 overflow-visible rounded-lg bg-[#f4f4f4] sm:h-12 lg:order-2 lg:max-w-[540px] xl:max-w-[640px]">
                        ...
                        />
                        <button type="submit" className="flex w-12 items-center justify-center text-[#07162b] transition hover:text-[#f58220] sm:w-14" aria-label="Search">
                            <SearchIcon />
                        </button>
                        ...
                        {isSearchFocused && hasSearch && (
                            <div className="absolute left-0 right-0 top-full z-[70] mt-2 overflow-hidden rounded-lg bg-white text-[#07162b] shadow-xl ring-1 ring-black/5">
                                {suggestions.length > 0 ? (
                                    ...
                                    <div className="px-4 py-4 text-sm text-slate-500">No matching products found.</div>
                                )}
                            </div>
                        )}
                    </form>

                    <nav aria-label="Account navigation" className="order-2 ml-auto grid flex-1 grid-cols-5 items-end gap-1 sm:flex-none sm:gap-3 lg:order-3 lg:flex lg:gap-5 xl:gap-7">
            ...
            <nav className="sticky top-0 z-50 bg-[#002b24] text-white shadow-md" aria-label="Product categories">
                <div className="mx-auto flex max-w-[1760px] flex-wrap items-center justify-center gap-x-3 gap-y-2 px-2.5 py-2.5 text-[11px] font-medium sm:gap-x-4 sm:px-4 sm:text-xs md:text-sm lg:justify-between lg:gap-x-4 lg:px-6 lg:text-[15px] xl:px-10">
                    {navItems.map(({ label, products }) => (
                        <div key={label} className="group relative">
                            <Link to={`/category/${slugify(label)}`} className="flex min-w-0 items-center gap-1.5 transition hover:text-[#ffb66d]">
                                <span>{label}</span>
                                ...
                                <div className="invisible absolute left-1/2 top-full z-50 mt-2 w-52 max-w-[calc(100vw-1rem)] -translate-x-1/2 rounded-md bg-white py-3 text-[#07162b] opacity-0 shadow-xl ring-1 ring-black/5 transition duration-200 group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:opacity-100 sm:w-56">
                                    ...
                                    <ul className="relative space-y-1">
                                        {products.map((product) => (
                                            <li key={product}>
                                                <Link to={`/category/${slugify(label)}/${slugify(product)}`} className="block px-4 py-2 text-sm font-medium transition hover:bg-[#fff4ea] hover:text-[#f58220]">
                                                    {product}
                                                </Link>
                                            </li>
                                        ))}
