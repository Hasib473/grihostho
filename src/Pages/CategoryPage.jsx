...
const CategoryPage = () => {
    ...
    const selectedProduct = category?.products?.find((item) => slugify(item) === productSlug);
    const visibleItems = selectedProduct ? [selectedProduct] : category?.products || [category?.label].filter(Boolean);
    ...
    if (!category) {
        return (
            <main className="mx-auto min-h-[420px] max-w-[1760px] px-4 py-12 sm:px-6 lg:px-10">
                ...
                <h1 className="mt-2 text-3xl font-bold text-[#07162b]">Category not found</h1>
    ...
    if (productSlug && !selectedProduct) {
        return (
            <main className="mx-auto min-h-[420px] max-w-[1760px] px-4 py-12 sm:px-6 lg:px-10">
                <p className="text-sm font-semibold uppercase tracking-wide text-[#f58220]">Not Found</p>
                <h1 className="mt-2 text-3xl font-bold text-[#07162b]">Product not found</h1>
                <Link to={`/category/${category.slug}`} className="mt-6 inline-flex rounded bg-[#f58220] px-5 py-2.5 font-semibold text-white transition hover:bg-[#e77412]">
        ...
        <main className="mx-auto min-h-[420px] max-w-[1760px] px-3 py-6 sm:px-6 sm:py-10 lg:px-10">
            <div className="rounded-lg bg-[#fff8f1] px-4 py-6 sm:px-8 sm:py-8">
                <div className="flex flex-wrap items-center gap-2 text-sm font-medium text-slate-500">
                    <Link to="/" className="transition hover:text-[#f58220]">Home</Link>
                    <span>/</span>
                    <Link to={`/category/${category.slug}`} className="transition hover:text-[#f58220]">{category.label}</Link>
                    {selectedProduct && (
                        <>
                            <span>/</span>
                            <span className="text-[#f58220]">{selectedProduct}</span>
                        </>
                    )}
                </div>
                ...
                <p className="mt-3 max-w-2xl text-sm leading-6 text-slate-600 sm:text-base">
                    Explore safe, reliable, and carefully selected {selectedProduct || category.label} items from Grihostho.
                </p>
            </div>

            <section className="mt-8">
                <h2 className="text-xl font-bold text-[#07162b]">
                    {selectedProduct ? 'Available Item' : `Shop ${category.label}`}
                </h2>
                ...
                <div className="mt-5 grid gap-4 min-[520px]:grid-cols-2 lg:grid-cols-4">
                    {visibleItems.map((item) => (
                        <Link
                            key={item}
                            to={`/product/${category.slug}/${slugify(item)}`}
                        ...
                        >
                            ...
                            <h3 className="mt-4 text-base font-semibold text-[#07162b]">{item}</h3>
